...
scenario_config_profile_switch() {
    ...
    local list_json
    list_json=$("$FT_BIN" config profile list --json --path "$config_path")
...
scenario_ruleset_profile_switch() {
    ...
    local list_out
    list_out=$("$FT_BIN" rules profile list -f json --path "$config_path" 2>&1) || true
    e2e_add_file "ruleset_profile_list.json" "$list_out"

    # Apply the incident ruleset profile
    local apply_out
    apply_out=$("$FT_BIN" rules profile apply incident --path "$config_path" 2>&1) || true
    e2e_add_file "ruleset_apply_output.txt" "$apply_out"

    # List again to confirm the active profile changed
    local list_after
    list_after=$("$FT_BIN" rules profile list -f json --path "$config_path" 2>&1) || true
...
scenario_pane_bookmarks() {
    ...
    add1=$("$FT_BIN" panes bookmark add 1 --alias build --tags "ci,dev" --description "Build pane" --json)
    e2e_add_file "bookmark_add_1.json" "$add1"
    echo "$add1" | jq -e '.ok == true' >/dev/null

    add2=$("$FT_BIN" panes bookmark add 2 --alias tests --tags "ci" --description "Test runner" --json)
    e2e_add_file "bookmark_add_2.json" "$add2"
    echo "$add2" | jq -e '.ok == true' >/dev/null

    add3=$("$FT_BIN" panes bookmark add 3 --alias logs --tags "ops,monitoring" --description "Log viewer" --json)
    e2e_add_file "bookmark_add_3.json" "$add3"
    echo "$add3" | jq -e '.ok == true' >/dev/null

    # List all bookmarks (output is a bare JSON array)
    local list_all
    list_all=$("$FT_BIN" panes bookmark list --json)
    ...
    local list_ci
    list_ci=$("$FT_BIN" panes bookmark list --tag ci --json)
    ...
    local list_ops
    list_ops=$("$FT_BIN" panes bookmark list --tag ops --json)
...
scenario_combined_profile_and_bookmarks() {
    ...
    write_file "$profile_path" "[general]\nlog_level = \"trace\"\n"
    "$FT_BIN" config profile apply dev --path "$config_path"
    ...
    # Add bookmarks
    "$FT_BIN" panes bookmark add 10 --alias dev-shell --tags "dev" --json | jq -e '.ok == true' >/dev/null
    "$FT_BIN" panes bookmark add 11 --alias dev-logs --tags "dev,monitoring" --json | jq -e '.ok == true' >/dev/null

    # List bookmarks (JSON array) — should have 2
    local bm_list
    bm_list=$("$FT_BIN" panes bookmark list --json)
    e2e_add_file "combined_bookmark_list.json" "$bm_list"
    echo "$bm_list" | jq -e 'length == 2' >/dev/null

    # Human-readable list should also work
    local bm_human
    bm_human=$("$FT_BIN" panes bookmark list)
    ...
    "$FT_BIN" config profile rollback --yes --path "$config_path"
    local restored
    restored=$(cat "$config_path")
    ...
    local bm_after
    bm_after=$("$FT_BIN" panes bookmark list --json)
