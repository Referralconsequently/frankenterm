...
rch_init "${ARTIFACT_DIR}" "${RUN_ID}" "ft_akx00_7_4_ssh_match_exec"
...
echo "=== ${BEAD_ID} SSH Match Exec verification ==="
echo "Artifacts: ${ARTIFACT_DIR}"

run_cargo_step "ssh_match_exec_tests" test -p frankenterm-ssh match_exec -- --nocapture
...
run_shell_step "placeholder_audit" \
  "! rg -n 'Match Exec is not implemented' frankenterm/ssh/src/config.rs"
run_shell_step "surface_audit" \
  "rg -n 'MatchExecPolicy|MatchExecOutcome|MatchExecDiagnostic|for_host_with_match_diagnostics' frankenterm/ssh/src/config.rs"
