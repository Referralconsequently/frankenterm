...
(
  cd "${ROOT_DIR}"
  CARGO_TARGET_DIR="target-e2e-1i2ge-6-7-${RUN_ID}" \
    cargo check -p frankenterm-core --features subprocess-bridge \
    --test mission_telemetry_integrity 2>&1
) > "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.compile.log" 2>&1
compile_rc=$?
set -e
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-6-7-${RUN_ID}" \
    ...
    --test mission_telemetry_integrity 2>&1
) > "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests.log" 2>&1
test_rc=$?
set -e
...
telemetry_count=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests.log" || echo 0)
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-6-7-${RUN_ID}" \
    ...
    --test mission_telemetry_integrity 2>&1
) > "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.clippy.log" 2>&1
clippy_rc=$?
set -e

telemetry_warnings=$(grep -c "mission_telemetry_integrity.rs" "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.clippy.log" || echo 0)
...
for pattern in \
  ...
  "trust_" \
  "determinism_"; do
  if ! grep -q "${pattern}.*ok" "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests.log"; then
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-6-7-${RUN_ID}" \
    cargo test -p frankenterm-core --features subprocess-bridge \
    --test mission_telemetry_integrity 2>&1
) > "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests_repeat.log" 2>&1
repeat_rc=$?
set -e
...
pass_count_1=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests.log" || echo 0)
pass_count_2=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_6_7_${RUN_ID}.tests_repeat.log" || echo 0)
...
rm -rf "${ROOT_DIR}/target-e2e-1i2ge-6-7-${RUN_ID}" 2>/dev/null || true
