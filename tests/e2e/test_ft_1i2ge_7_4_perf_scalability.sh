...
(
  cd "${ROOT_DIR}"
  CARGO_TARGET_DIR="target-e2e-1i2ge-7-4-${RUN_ID}" \
    cargo check -p frankenterm-core --features subprocess-bridge \
    --test mission_perf_scalability 2>&1
) > "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.compile.log" 2>&1
compile_rc=$?
set -e

if [[ ${compile_rc} -ne 0 ]]; then
  emit_log "failed" "compile_check" "compilation_error" "COMPILE_FAIL" \
    "ft_1i2ge_7_4_${RUN_ID}.compile.log" "cargo check failed"
  echo "FAIL: compilation error" >&2
  exit 1
fi
emit_log "passed" "compile_check" "compilation_ok" "none" \
  "ft_1i2ge_7_4_${RUN_ID}.compile.log" "compilation succeeded"
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-7-4-${RUN_ID}" \
    ...
    --test mission_perf_scalability 2>&1
) > "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests.log" 2>&1
test_rc=$?
set -e

if [[ ${test_rc} -ne 0 ]]; then
  emit_log "failed" "perf_tests" "test_failure" "TEST_FAIL" \
    "ft_1i2ge_7_4_${RUN_ID}.tests.log" "perf scalability tests failed"
  echo "FAIL: perf scalability tests" >&2
  exit 1
fi

perf_count=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests.log" || echo 0)

if [[ ${perf_count} -lt 20 ]]; then
  emit_log "failed" "perf_tests" "insufficient_test_coverage" "COVERAGE_LOW" \
    "ft_1i2ge_7_4_${RUN_ID}.tests.log" \
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-7-4-${RUN_ID}" \
    ...
    --test mission_perf_scalability 2>&1
) > "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.clippy.log" 2>&1
clippy_rc=$?
set -e

perf_warnings=$(grep -c "mission_perf_scalability.rs" "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.clippy.log" || echo 0)
if [[ ${perf_warnings} -gt 0 ]]; then
  emit_log "failed" "clippy_check" "clippy_warnings" "CLIPPY_WARN" \
    "ft_1i2ge_7_4_${RUN_ID}.clippy.log" \
...
emit_log "passed" "clippy_check" "clippy_clean" "none" \
  "ft_1i2ge_7_4_${RUN_ID}.clippy.log" "zero clippy warnings"
...
for pattern in \
  ...
  "trigger_" \
  "determinism_"; do
  if ! grep -q "${pattern}.*ok" "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests.log"; then
...
emit_log "passed" "category_coverage" "all_categories_covered" "none" \
  "ft_1i2ge_7_4_${RUN_ID}.tests.log" "all budget categories covered"
...
(
  ...
  CARGO_TARGET_DIR="target-e2e-1i2ge-7-4-${RUN_ID}" \
    cargo test -p frankenterm-core --features subprocess-bridge \
    --test mission_perf_scalability 2>&1
) > "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests_repeat.log" 2>&1
repeat_rc=$?
set -e

if [[ ${repeat_rc} -ne 0 ]]; then
  emit_log "failed" "determinism" "repeat_run_failed" "REPEAT_FAIL" \
    "ft_1i2ge_7_4_${RUN_ID}.tests_repeat.log" "repeat run failed"
  echo "FAIL: repeat test run" >&2
  exit 1
fi

pass_count_1=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests.log" || echo 0)
pass_count_2=$(grep -c "ok$" "${LOG_DIR}/ft_1i2ge_7_4_${RUN_ID}.tests_repeat.log" || echo 0)
if [[ ${pass_count_1} -ne ${pass_count_2} ]]; then
  emit_log "failed" "determinism" "count_mismatch" "DETERMINISM_FAIL" \
    "ft_1i2ge_7_4_${RUN_ID}.tests_repeat.log" \
...
rm -rf "${ROOT_DIR}/target-e2e-1i2ge-7-4-${RUN_ID}" 2>/dev/null || true
