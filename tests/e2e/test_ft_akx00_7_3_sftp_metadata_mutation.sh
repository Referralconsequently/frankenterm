...
rch_init "${ARTIFACT_DIR}" "${RUN_ID}" "ft_akx00_7_3_sftp_metadata_mutation"
...
echo "=== ${BEAD_ID} SFTP metadata mutation verification ==="
echo "Artifacts: ${ARTIFACT_DIR}"

run_cargo_step "sftp_file_metadata_tests" test -p frankenterm-ssh set_metadata_should_ -- --nocapture
run_cargo_step "ssh_check_all_targets" check -p frankenterm-ssh --all-targets
run_cargo_step "ssh_clippy_all_targets" clippy --no-deps -p frankenterm-ssh --all-targets -- -D warnings
run_cargo_step "workspace_fmt_check" fmt --check
run_shell_step "placeholder_audit" \
  "! rg -n 'FileWrap::set_metadata not implemented for libssh::SftpFile' frankenterm/ssh/src/filewrap.rs"
run_shell_step "surface_audit" \
  "rg -n 'libssh_path|access-time changes|SetMetadata\\(|set_metadata_should_(update_|reject_)' frankenterm/ssh/src/filewrap.rs frankenterm/ssh/src/sessioninner.rs frankenterm/ssh/src/sftpwrap.rs frankenterm/ssh/tests/e2e/sftp/file.rs"
