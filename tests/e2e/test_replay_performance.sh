...
run_bench() {
  local scenario="$1"
  local bench_name="$2"
  local stdout_file="$raw_dir/scenario${scenario}.${bench_name}.stdout.log"
  local stderr_file="$raw_dir/scenario${scenario}.${bench_name}.stderr.log"

  set +e
  rch exec -- env \
    CARGO_HOME="$cargo_home" \
    CARGO_TARGET_DIR="$cargo_target_dir" \
    cargo bench -p frankenterm-core --bench "$bench_name" \
    >"$stdout_file" 2>"$stderr_file"
  local rc=$?
  set -e

  if grep -Fq "[RCH] local" "$stderr_file"; then
    emit_metric_log "$scenario" "${bench_name}_offload" "0" "1" "false" "fail" "rch_local_fallback" "${stderr_file#$ROOT_DIR/}"
    suite_status=1
  ...
  if [[ $rc -ne 0 ]]; then
    emit_metric_log "$scenario" "${bench_name}_execution" "0" "1" "false" "fail" "cargo_bench_failed" "${stderr_file#$ROOT_DIR/}"
...
set +e
rch workers probe --all >"$probe_log" 2>&1
probe_rc=$?
set -e
if [[ $probe_rc -ne 0 ]] || ! grep -q "✓" "$probe_log"; then
  emit_metric_log "0" "rch_workers_probe" "0" "1" "false" "fail" "rch_workers_unreachable" "${probe_log#$ROOT_DIR/}"
  exit 1
fi
