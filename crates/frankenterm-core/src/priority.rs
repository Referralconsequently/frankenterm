...
pub fn shedding_order(priorities: &HashMap<u64, PanePriority>) -> Vec<u64> {
    let mut panes: Vec<(u64, PanePriority)> = priorities
        .iter()
        .map(|(&id, &p)| (id, p))
        .collect();
...
mod tests {
    ...
    fn rate_tracker_decays_over_time() {
        ...
        let t2 = t1 + half_life;
        let rate_after_hl = tracker.lines_per_second(t2);
        let ratio = rate_after_hl / rate_at_burst;
        assert!(
            (ratio - 0.5).abs() < 0.05,
        ...
        let rate_after_3hl = tracker.lines_per_second(t3);
        let ratio3 = rate_after_3hl / rate_at_burst;
        assert!(
    ...
    fn classifier_high_rate_produces_high() {
        ...
        assert_eq!(c.classify_at(1, now + Duration::from_secs(1)), PanePriority::High);
    }
