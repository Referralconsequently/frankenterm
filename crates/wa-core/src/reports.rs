...
use crate::storage::{
    AuditQuery, EventQuery, ExportQuery, StorageHandle,
};
...
fn format_ts(epoch_ms: i64) -> String {
    ...
    format!(
        "{year:04}-{month:02}-{day:02} {hours:02}:{minutes:02}:{seconds:02}.{millis:03}Z"
    )
}
...
mod tests {
    ...
    fn format_ts_basic() {
        ...
        assert!(formatted.contains("2025-01-15"), "Expected date 2025-01-15, got: {formatted}");
        assert!(formatted.contains("12:00:45.123Z"), "Expected time, got: {formatted}");
    }
    ...
    async fn report_with_workflows() {
        ...
        storage
            .insert_step_log(
                "wf-auth-1", None, 0, "wait_for_prompt", None, None,
                "continue", None, None, None, None, 1000, 1500,
            )
            .await
            .unwrap();
        storage
            .insert_step_log(
                "wf-auth-1", None, 1, "send_credentials", None, None,
                "done", None, None, None, None, 1500, 3000,
            )
            .await
            .unwrap();
    ...
    async fn report_full_fixture() {
        ...
        // Segment + gap
        storage.append_segment(1, "initial output", None).await.unwrap();
        ...
        storage
            .insert_step_log(
                "wf-full-1", None, 0, "detect_prompt", None, None,
                "continue", None, None, None, None, 2000, 2500,
            )
            .await
            .unwrap();
        storage
            .insert_step_log(
                "wf-full-1", None, 1, "authenticate", None, None,
                "done", None, None, None, None, 2500, 4000,
            )
            .await
            .unwrap();
