...
- Persistence: `pane_uuid` is stored in the `panes` table and surfaced in
  `ft status` / `ft robot state` once the watcher has observed the pane.
...
- Use `ft status` or `ft robot state` to confirm the `pane_uuid` associated
  with a pane.
...
|---------|---------|---------|
| state | List all panes | `ft robot state` |
| get-text | Read pane output | `ft robot get-text 3 --last-n-lines 50` |
| send | Send input | `ft robot send 3 "ls -la"` |
| wait-for | Wait for pattern | `ft robot wait-for 3 "done" --timeout-secs 60` |
| search | Search history | `ft robot search "error"` |
| events | List events | `ft robot events --unhandled-only` |
| workflow | Run workflow | `ft robot workflow handle_usage_limits 3` |
...
pub async fn run(format: &super::OutputFormat) -> Result<()> {
    let help = r#"
╔══════════════════════════════════════════════════════════════════════════════╗
║                    ft - FrankenTerm v0.1.0                              ║
║                    AI Coding Agent Fleet Manager                              ║
╚══════════════════════════════════════════════════════════════════════════════╝

QUICK START
-----------
  ft watch           Start daemon (monitors all panes, auto-handles events)
  ft status          Show current pane states
  ft robot help      Agent-optimized interface documentation

CORE COMMANDS
-------------
  ft watch [--interval-ms 500]     Continuous monitoring daemon
  ft robot <cmd>                   Agent-first JSON interface
  ft query <search-term>           Search all captured output
  ft send <pane-id> <text>         Send input to pane
  ft workflow <name> <pane-id>     Execute predefined workflow

ROBOT MODE (for AI agents)
--------------------------
  ft robot state                   JSON list of all panes
  ft robot get-text <pane-id>      Get pane content
  ft robot send <pane-id> <text>   Send input
  ft robot wait-for <pane-id> <pattern>  Wait for output pattern
  ft robot search <query>          Search history (FTS5)
  ft robot events                  List detected events
  ft robot workflow <name> <pane>  Execute workflow

SETUP
-----
  ft setup local                   Configure local WezTerm
  ft setup remote <host>           Configure remote mux server
  ft setup accounts                Manage service accounts
...
--------
  ...
  ft send 3 "proceed with the implementation"

  # Search for errors across all history
  ft query "error OR failed OR exception"
...
- `ft robot state` shows all panes
