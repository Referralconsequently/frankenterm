...
| `RC-INTERACTION-001` | Interaction continuity + safety | Interactive workflows remain stable through resize events and unsafe sends are blocked when pane state is unsafe | `crates/frankenterm-core/src/workflows.rs`, `crates/frankenterm-core/tests/resize_scheduler_state_machine_tests.rs` | `docs/resize-performance-slos.md` |
