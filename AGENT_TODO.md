...
- [x] Identify entry points (`crates/frankenterm/src/main.rs`)
...
- [x] Review tests layout (crates/frankenterm-core/tests, benches, fuzz)
...
### 4.0 Discovery & Setup
- [x] Confirm manifests: root + crates/frankenterm + crates/frankenterm-core + fuzz
...
- [x] Extend migration model (up/down, plan, status) in `crates/frankenterm-core/src/storage.rs`
- [x] Wire CLI: `ft db migrate` with `--status`, `--run`, `--to <version>`
...
- [x] List active agents (`resource://agents/users-jemanuel-projects-frankenterm`)
...
- [x] Use `ft events -f json --unhandled` to assert exactly 1 relevant event
- [x] Use `ft robot events --unhandled --would-handle --dry-run` to fetch recommended workflow
...
- [ ] Run `cargo fmt --check` (failed; formatting diffs in frankenterm-core files not touched)
- [x] Run `cargo check --all-targets`
- [ ] Run `cargo clippy --all-targets -- -D warnings` (failed: clippy::needless_raw_string_hashes in frankenterm-core/desktop_notify.rs)
...
## 2) Implement wa-1pe.3: `ft workflow run --dry-run`
...
- [x] Created `crates/frankenterm-core/tests/dry_run_integration.rs` with 33 tests across 7 categories
...
- [x] Created `crates/frankenterm-core/tests/noise_control_tests.rs` with 34 tests
...
- [x] Added `render_with_noise_info()` to EventListRenderer (renderers.rs)
  ...
  - Footer shows active mute count with "ft mute list" hint
- [x] Updated `ft events` handler to fetch mutes and use `render_with_noise_info`
- [x] Updated `ft triage` handler with noise control integration:
...
- [x] Created `crates/frankenterm-core/tests/incident_bundle_tests.rs` with 26 integration tests
...
## 10) Implement wa-upg.1.4: `ft reproduce replay` (deterministic incident replayer)
- [x] Converted `ft reproduce` from flat command to subcommand structure: `export` + `replay`
- [x] Added `ReproduceCommands` enum with `Export` (backward-compatible) and `Replay` subcommands
- [x] Updated `ft reproduce export` to use `collect_incident_bundle` (enhanced collector with DB metadata)
...
- [x] Updated all `ft reproduce --kind crash` references to `ft reproduce export --kind crash`
...
- [x] Created `crates/frankenterm-core/benches/storage_regression.rs` with 5 benchmark groups:
...
- [x] Added `[[bench]] name = "storage_regression" harness = false` to crates/frankenterm-core/Cargo.toml
...
- [x] Created `crates/frankenterm-core/tests/usage_metrics_tests.rs` with 15 tests covering type roundtrips, CRUD, filtering, aggregation, retention, schema migration
...
- [x] Created `crates/frankenterm-core/tests/notification_history_tests.rs` with 24 tests covering status roundtrips, CRUD, filtering (channel/status/event_id/time_range/combined), purge, retry, acknowledgement, schema migration
...
- [x] Created `crates/frankenterm-core/src/alerts.rs` module (~500 lines)
...
- [x] Created `crates/frankenterm-core/tests/alert_monitor_tests.rs` with 11 integration tests: cost alerts (threshold/below/exceeded), token alerts, rate limit alerts, multiple rules, disabled rules, empty DB, old metrics outside window, summary formatting, account balance with no service
...
- [x] Created `crates/frankenterm-core/tests/event_annotation_tests.rs` with 21 integration tests:
...
## 16) Implement wa-985.3: Analytics CLI commands (ft analytics)
...
- [x] Added analytics renderers in `crates/frankenterm-core/src/output/renderers.rs`:
...
- [x] Updated `crates/frankenterm-core/src/output/mod.rs` with new public exports
...
- [x] Created `crates/frankenterm-core/tests/analytics_cli_tests.rs` with 15 integration tests:
