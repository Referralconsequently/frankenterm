...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(frankenterm_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| frankenterm_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        frankenterm_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
    ...
    let tokens = quote! {
        impl #impl_generics frankenterm_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &frankenterm_dynamic::Value, options: frankenterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, frankenterm_dynamic::Error> {
                use frankenterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(frankenterm_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| frankenterm_dynamic::Error::Message(
    ...
    let tokens = quote! {
        impl frankenterm_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &frankenterm_dynamic::Value, options: frankenterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, frankenterm_dynamic::Error> {
                use frankenterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
