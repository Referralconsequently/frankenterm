...
impl FromDynamic for IntegratedTitleButtonStyle {
        ...
        Self: Sized,
    {
        ...
        if let frankenterm_dynamic::Value::String(string) = value {
            let style = match string.as_str() {
                "Windows" => Self::Windows,
                "Gnome" => Self::Gnome,
                "MacOsNative" if cfg!(target_os = "macos") => Self::MacOsNative,
                _ => {
                    return Err(frankenterm_dynamic::Error::InvalidVariantForType {
...
mod test {
    ...
    fn modifiers_remove_positional_mods() {
        let mods =
            Modifiers::CTRL | Modifiers::LEFT_CTRL | Modifiers::SHIFT | Modifiers::LEFT_SHIFT;
