...
impl SessionInner {
    ...
    fn run_impl(&mut self) -> anyhow::Result<()> {
        ...
        match backend {
            ...
            "ssh2" => anyhow::bail!(
                "invalid frankenterm_ssh_backend value: {}, not compiled with `ssh2`",
                backend
            ),
            ...
            #[cfg(not(feature = "libssh-rs"))]
            "libssh" => anyhow::bail!(
                "invalid frankenterm_ssh_backend value: {}, not compiled with `libssh`",
                backend
            ),

            _ => anyhow::bail!(
                "invalid frankenterm_ssh_backend value: {}, expected either `ssh2` or `libssh`",
                backend
            ),
        }
    }
    ...
    fn connect_pending_agent_forward_channels(&mut self, sess: &mut SessionWrap) {
        fn process_one(sess: &mut SessionInner, channel: ChannelWrap) -> anyhow::Result<()> {
            ...
            let mut fd = {
                use frankenterm_uds::UnixStream;
                #[cfg(unix)]
                {
